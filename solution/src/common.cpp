...
std::map<string, Weights> loadWeights(const string& file) {
    ...
    assert(input.is_open() && "unable to load weights file");

    // read number of weight blobs
    int32_t count;
    input >> count;
    assert(count > 0 && "invalid weights map file");
...
ILayer* convBnLeaky(INetworkDefinition* network, std::map<string, Weights>& weightMap,
    ...
    int p = ksize / 2;
    IConvolutionLayer* conv1 = network->addConvolutionNd(input, outch,
        DimsHW{ksize, ksize}, weightMap[lname + ".conv.weight"], emptywts);
    assert(conv1);
...
ILayer* focus(INetworkDefinition* network, std::map<string, Weights>& weightMap,
    ...
    ISliceLayer *s1 = network->addSlice(input,
        Dims3{0, 0, 0}, Dims3{inch, ih / 2, iw / 2}, Dims3{1, 2, 2});
    ISliceLayer *s2 = network->addSlice(input,
        Dims3{0, 1, 0}, Dims3{inch, ih / 2, iw / 2}, Dims3{1, 2, 2});
    ISliceLayer *s3 = network->addSlice(input,
        Dims3{0, 0, 1}, Dims3{inch, ih / 2, iw / 2}, Dims3{1, 2, 2});
    ISliceLayer *s4 = network->addSlice(input,
        Dims3{0, 1, 1}, Dims3{inch, ih / 2, iw / 2}, Dims3{1, 2, 2});
...
ILayer* bottleneckCSP(INetworkDefinition* network, std::map<string, Weights>& weightMap,
    ...
    auto cv2 = network->addConvolutionNd(input, c_,
        DimsHW{1, 1}, weightMap[lname + ".cv2.weight"], emptywts);
    ITensor* y1 = cv1->getOutput(0);
    for (int i = 0; i < n; i++) {
        auto b = bottleneck(network, weightMap, *y1,
            c_, c_, shortcut, g, 1.0, lname + ".m." + std::to_string(i));
        y1 = b->getOutput(0);
    }
    auto cv3 = network->addConvolutionNd(*y1, c_,
       DimsHW{1, 1}, weightMap[lname + ".cv3.weight"], emptywts);
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition* network, std::map<std::string, Weights>& weightMap, IConvolutionLayer* det0, IConvolutionLayer* det1, IConvolutionLayer* det2)
{
    ...
    NetData[1] = Yolo::Y_INPUT_W;
    NetData[2] = Yolo::Y_INPUT_H;
    ...
    for (int k = 1; k < 4; k++)
    {
        plugindata[k - 1][0] = Yolo::Y_INPUT_W / scale[k - 1];
        plugindata[k - 1][1] = Yolo::Y_INPUT_H / scale[k - 1];
        for (int i = 2; i < 8; i++)
        {
...
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {
    int l, r, t, b;
    float r_w = Yolo::Y_INPUT_W / (img.cols * 1.0);
    float r_h = Yolo::Y_INPUT_H / (img.rows * 1.0);
    if (r_h > r_w) {
        l = bbox[0] - bbox[2] / 2.f;
        r = bbox[0] + bbox[2] / 2.f;
        t = bbox[1] - bbox[3] / 2.f - (Yolo::Y_INPUT_H - r_w * img.rows) / 2;
        b = bbox[1] + bbox[3] / 2.f - (Yolo::Y_INPUT_H - r_w * img.rows) / 2;
        l = l / r_w;
        r = r / r_w;
        t = t / r_w;
        b = b / r_w;
        ...
        l = bbox[0] - bbox[2] / 2.f - (Yolo::Y_INPUT_W - r_h * img.cols) / 2;
        r = bbox[0] + bbox[2] / 2.f - (Yolo::Y_INPUT_W - r_h * img.cols) / 2;
        t = bbox[1] - bbox[3] / 2.f;
        b = bbox[1] + bbox[3] / 2.f;
        l = l / r_h;
        r = r / r_h;
